-- 庫存來源商品:禮盒可設定「賣出時改扣指定商品的庫存」(如 9入蔥餅禮盒扣蔥餅袋),
-- 營收與報表仍歸禮盒商品本身;袋與盒共用同一套庫存,不需分開管理。

alter table public.products
  add column stock_source_product_id uuid references public.products(id);
...
begin
  ...
  loop
    ...
    -- 設定「庫存來源商品」者改扣來源商品(如蔥餅禮盒扣蔥餅袋)。
    v_own_stock := v_product.category = 'bag'
      or (v_product.category = 'gift_box' and coalesce(v_product.selection_mode, 'fixed') <> 'select');

    if v_own_stock then
      v_stock_product_id := coalesce(v_product.stock_source_product_id, v_product.id);
      v_stock := public.current_stock(p_counter_id, v_stock_product_id, null);
      ...
      if v_sale_qty > 0 then
        insert into public.inventory_movements (
          ...
          p_counter_id, v_stock_product_id, 'sale', -v_sale_qty, p_actor_id,
          'POS 訂單 ' || p_order_no ||
            case when v_stock_product_id <> v_product.id then '（' || v_product.name || '）' else '' end,
          p_order_id
        );
      end if;
