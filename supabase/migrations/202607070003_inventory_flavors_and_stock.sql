-- 庫存品項支援禮盒口味(6入/袋)、記錄異動編修者、庫存不足擋單。
-- 禮盒本身不入庫存:訂單售出禮盒時改扣口味(與蔥軋餅商品)庫存。

alter table public.inventory_movements
  alter column product_id drop not null;

alter table public.inventory_movements
  add column flavor_id uuid references public.flavors(id),
  add column updated_by uuid references public.profiles(id),
  add column updated_at timestamptz;

alter table public.inventory_movements
  add constraint inventory_movements_single_item check (
    ((product_id is not null)::int + (flavor_id is not null)::int) = 1
  );
...
as $$
  with target as (
    select quantity, counted_quantity, created_at
      from public.inventory_movements
      where counter_id = p_counter_id
        and (
          (p_product_id is not null and product_id = p_product_id)
          or (p_flavor_id is not null and flavor_id = p_flavor_id)
        )
  ),
  last_count as (
    select counted_quantity, created_at
      from target
  ...
  select coalesce(
    (
      select lc.counted_quantity + coalesce((
        select sum(t.quantity) from target t where t.created_at > lc.created_at
      ), 0)
      from last_count lc
    ),
    (select coalesce(sum(quantity), 0) from target)
  )::integer;
$$;

grant execute on function public.current_stock(uuid, uuid, uuid) to authenticated, service_role;
  ...
  loop
    ...
    if v_product.category = 'bag' then
      ...
      insert into public.inventory_movements (
        counter_id, product_id, movement_type, quantity, created_by, note
      ) values (
        p_counter_id, v_product.id, 'sale', -v_quantity, p_cashier_id, 'POS 訂單 ' || v_order_no
      );
    end if;
    ...
    loop
      ...
      if v_product.category = 'gift_box' then
        ...
        if v_flavor_id is not null then
          ...
          insert into public.inventory_movements (
            counter_id, flavor_id, movement_type, quantity, created_by, note
          ) values (
            p_counter_id, v_flavor_id, 'sale', -v_gift_quantity, p_cashier_id,
            'POS 訂單 ' || v_order_no || '（' || v_product.name || '）'
          );
        end if;
      end if;
    end loop;
    ...
    if v_product.category = 'gift_box' and coalesce(v_product.includes_scallion_cracker, false) then
      ...
      if found then
        ...
        insert into public.inventory_movements (
          ...
          'POS 訂單 ' || v_order_no || '（' || v_product.name || ' 附贈）'
        );
      end if;
    end if;
  end loop;
