...
export async function GET(request: Request) {
  ...
  let query = supabase
    .from("orders")
    .select(
      "id, order_no, created_at, counter_id, seller_id, cashier_id, discount_id, payment_method, sales_amount, discount_amount, receivable_amount, received_amount, status, void_reason, counters(name), seller:profiles!orders_seller_id_fkey(display_name), cashier:profiles!orders_cashier_id_fkey(display_name), voider:profiles!orders_voided_by_fkey(display_name), editor:profiles!orders_edited_by_fkey(display_name)"
    )
  ...
  if (isStaff && guard.profile) {
    query = query.or(`cashier_id.eq.${guard.profile.id},seller_id.eq.${guard.profile.id}`);
  }
      ...
      orders: (orders ?? []).map((order) => ({
        ...
        canVoid:
          ...
          (!guard.profile ||
            ...
            (order.cashier_id === guard.profile.id &&
...
export async function PATCH(request: Request) {
  ...
  const { data: order, error: orderError } = await supabase
    .from("orders")
    .select("id, order_no, cashier_id, created_at, status")
    .eq("id", input.orderId)
    .single();
  ...
  // 一般員工只能作廢「自己收銀、當日」的訂單;店長不受限。原單保留供稽核。
  if (guard.profile && guard.profile.role !== "manager") {
    if (order.cashier_id !== guard.profile.id) {
...
export async function POST(request: Request) {
  ...
  const supabase = createSupabaseAdminClient();
  const cashierId =
    guard.profile?.id ?? input.cashierId ?? process.env.DEMO_CASHIER_ID ?? defaultCashierId;
...
function validateOrderInput(input: CreateOrderInput, options: { validateCatalog: boolean }) {
  ...
  if (!input.sellerId) return { ok: false as const, error: "缺少銷售人員" };
